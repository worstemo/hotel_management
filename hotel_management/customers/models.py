...
import re
# 导入datetime模块，用于验证身份证号中的出生日期是否有效
...
def validate_id_number(value):
    ...
    # 第二步：验证出生日期的有效性
    # ----------------------------------------------------------
    # 正则表达式只能验证日期格式（如01-31），但无法验证日期是否真实有效
    # 例如：2月30日、4月31日 会通过正则但实际不存在
    # 使用Python的datetime模块可以精确验证日期有效性（包括闰年判断）
    #
    # 身份证号结构：前6位地址码 + 8位出生日期(YYYYMMDD) + 3位顺序码 + 1位校验码
    # 所以：value[6:10]是年份，value[10:12]是月份，value[12:14]是日期
    #
    year = int(value[6:10])    # 提取第7-10位：出生年份（如1990）
    month = int(value[10:12])  # 提取第11-12位：出生月份（如03）
    day = int(value[12:14])    # 提取第13-14位：出生日期（如15）
    
    try:
        # datetime(year, month, day) 会尝试创建日期对象
        # 如果日期无效（如2月30日），会抛出ValueError异常
        # Python的datetime会自动处理闰年判断（如2000年2月29日有效，1999年2月29日无效）
        datetime(year, month, day)
    except ValueError:
        # 捕获ValueError异常，说明日期无效
